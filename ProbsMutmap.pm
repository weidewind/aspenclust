...
use Parsers qw(parse_likelihoods parse_likelihoods_csv parse_fasta_as_likelihoods parse_likelihoods_as_fasta parse_tree parse_fasta parse_splits);
...
sub get_output_base {
	my $args = shift;
	my $output_base = File::Spec->catdir(getcwd(), "output", $args->{input}, $args->{splits}, $args->{bigtag}, $args->{state}, likelihood_tag($args->{likelihood}));
	return $output_base;
}
		...
		if ($args->{fromcsv}){
			...
			($internalseqs, $keystring) = parse_likelihoods($probsfile); # $nodeseqs{$nodename}[$ind] = (0,0,0.8,0.2)
                ...
                if ($args->{fromcsv}){
	                my $probsfile = File::Spec->catfile($input_base, $args->{protein}.".ancestor.likelihoods");
        	        $treefile = File::Spec->catfile($input_base, $args->{protein}.".ancestor.likelihoods.withinternals.newick");
	                ($internalseqs, $keystring) = parse_likelihoods_as_fasta($probsfile.".csv"); # $nodeseqs{$nodename}[$ind] = (0,0,0.8,0.2)
                }
                else {
...
#		($internalseqs, $keystring) = parse_fasta_as_likelihoods($probsfile); # $nodeseqs{$nodename}[$ind] = (0,0,0.8,0.2)
	...
	my %node_partition;
	if ($args->{splits}){
		my $splitsdir = File::Spec->catfile($input_base, $args->{splits});
		my $splitsfile = File::Spec->catfile($splitsdir, $args->{protein}.".splits");
		%node_partition = parse_splits($splitsfile);
	}
	...
	$self = {
		...
		static_distance_in_date => $args->{distance_in_date},
		static_node_partition => \%node_partition,
	};
	bless $self, $class;
