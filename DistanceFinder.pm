   ...
   # }
	 ...
	 sub node_distance {
    	...
    	if  (exists $mutmap ->{static_distance_hash}{$node}{$other_node}){
			...
			my $dist;
			if ($date){
				$dist = calc_date_distance($node, $other_node);
			}
    		else{
				$dist = calc_nonzero_patristic_distance($node, $other_node);
			}
...
## If you give it two sequential nodes, it returns the distance between them 
 ...
 sub calc_nonzero_patristic_distance {
        my ( $node, $other_node ) = @_;
        my $patristic_distance = 0;
		if ($node->get_id eq $other_node->get_id){
			return $node->get_branch_length/3;
		}
...
sub calc_date_distance {
	my ( $node, $other_node ) = @_;
	return abs($node->get_branch_length-$other_node->get_branch_length);
}
