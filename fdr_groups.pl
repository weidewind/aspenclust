...
unless ($greponly){
	...
	FAKES:
	for my $fakenum (1..$fakenumber) {
		$pm->start and next FAKES;
		print "Starting fake $fakenum\n";
		srand($fakenum);
		Aspens::group_stats_batch({mutmap => $mutmap, shuffletype => "labelshuffler", simnumber => $simnumber, stattype => $stattype, norm => $norm, fake => $fakenum, groupnames => \%groupnames});
...
my $joined = File::Spec->catfile($results_folder, $protein."_".$state."_".$stattype."_group_FDR");
my $fmask = $protein."_".$state."_groups_labelshuffler_".$stattype."_FDR";
