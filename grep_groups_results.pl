...
my $input;
GetOptions (
                'input=s' =>\$input, # name of a folder inside output directory, which c$
        );
...
foreach my $filename(@files){
	if ($filename =~ /([a-zA-Z0-9]*)_groups_results_([a-z]+)/){
		print($input." ".$filename."\n");
		unlink File::Spec->catfile($input,$filename);
	}
}

foreach my $filename(sort @files){
	my $filepath =  File::Spec->catfile($input,$filename);
	next if (-d $filepath);
	...
	my $groupname = $3;
	my $stattype = $4;
	my $resfile = File::Spec->catfile($input, $prot."_groups_results_".$shufflertype);
