...
unless ($greponly){
		...
		for my $fakenum ($firstfake..($fakenumber+$firstfake-1)) {
			...
			srand($fakenum);
			Aspens::ancestors_stats({mutmap =>$mutmap, simnumber => $simnumber, sites => \@sites, verbose => $verbose, stattype => $stattype, norm =>$norm, fake => $fakenum});
...
my $fname = $protein."_".$state."_".$stattype."_ancestors_FDR";
my $fmask = $protein."_".$state."_ancestors_".$stattype."_statistics_FDR_";
