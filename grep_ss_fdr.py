...
output = os.path.join(args.analysis_folder,'Sites_with_FDR'+'.xlsx')
...
for prot in proteins:
    ...
    fdrfile = os.path.join(args.fdr_folder, prot+'_'+state+'_mean_sites_FDR')
    sitefile = os.path.join(args.analysis_folder,'Sites.xlsx')
    if not os.path.isfile(fdrfile):
        print ("No fdr file " + fdrfile)
        continue
    if not os.path.isfile(sitefile):
        print ("No site file " + sitefile)
        continue
    fdrdf = pd.read_csv(fdrfile,  delimiter=r"\s+", names= ["site", "par", "div", "diff", "pvalue"])
    print (fdrdf.head(5))
    sitesdf = pd.read_excel(sitefile,  sheet_name=prot)
    if (fdrdf.shape[0] == 0):
        print ("Fdr dataframe is empty!")
        continue
    if (sitesdf.shape[0] == 0):
        print ("sites dataframe is empty!")
        continue
    fdr = []
